...
class RegExPattern:
    ...
    def general_re_pattern():
        ...
        # the correct pattern
        pattern_correct = re.compile(r"^\(F\)[IV]{1,3}[abcde]?\s[1-9]\d*[a-z]?$")
        # a pattern with a leading zero
        pattern_leading_zero = re.compile(r"^\(F\)[IV]{1,3}[abcde]?\s0+[1-9]\d*[a-z]?$")
        # a pattern for the dummy inventarnummer
        pattern_dummy = re.compile(r"^\(F\)[a-zA-Z]*\sDU-\d+$")
    ...
    def ozeanien_re_pattern():
        ...
        # the correct pattern
        pattern_correct = re.compile(r"^\(F\)V[abc]\s[1-9]\d*[a-z]?$")
        # a pattern with a leading zero
        pattern_leading_zero = re.compile(r"^\(F\)V[abc]\s0+[1-9]\d*[a-z]?$")
    ...
    def inventar_sortierbar_re_pattern():
        ...
        pattern_no_letter = re.compile(r"^\(F\)[a-zA-Z]+\s\d*$")
        pattern_letter = re.compile(r"^\(F\)[a-zA-Z]+\s\d*[a-z]+$")
        # we need to differentiate between these two as otherwise the number of leading zeros would be incorrect
        pattern_letter_blank = re.compile(r"^\(F\)[a-zA-Z]+\s\d*\s[a-z]+$")
    ...
    def inschrift_re_pattern():
        ...
        """
        pattern_einlauf_correct = re.compile(r"^fot_[012]\d{3}$")
        ...
        pattern_incomplete = re.compile(r"^fot_\d{1,3}$")
        # to copy paste in code when using
